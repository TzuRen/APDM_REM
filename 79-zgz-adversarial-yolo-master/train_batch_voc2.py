...
import patch_config1 as patch_config
import sys
import time
...
class PatchTrainer(object):
    def __init__(self, mode):
        ...
        self.prob_extractor = MaxProbExtractor(1, 20, self.config).cuda()   #voc
    ...
    def train(self):
        ...
        for epoch in range(n_epochs):
            ...
            if True:
                finame='gen_adv_voc1/'+str(epoch)+'_0.png'
                im.save(finame)
                if tt>0:
                    del adv_batch_t, output, max_prob, det_loss, p_img_batch, nps_loss, tv_loss, loss
                torch.cuda.empty_cache()
