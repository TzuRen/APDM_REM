#encoding=utf-8
#对于所有的测试集，进行一次性所有类别的对抗块替换
#首先测试集图片是混杂的，图片里面的类别也是多样的，
#读取出数据时，需要根据其类别标签进行相应的选择对应对抗块，进行
#相应目标对应类别对抗块替换
...
class PatchTrainer(object):
    ...
    def train(self):
        ...
        train_loader = torch.utils.data.DataLoader(MyInriaDataset(self.config.img_dir, self.config.lab_dir, max_lab, img_size,
                                                                shuffle=True),
            ...
            for i_batch, (img_name,img_batch, lab_batch) in tqdm(enumerate(train_loader), desc=f'Running epoch {epoch}',
                ...
                with autograd.detect_anomaly():  #检测梯度异常值：无穷大/NAN(梯度爆炸/消失)
                    ...
                    for i in range(batch_size):
                        ...
                        file_name='gen_adv3/'+str(img_name[i])+'.png'
                        img.save(file_name)
    ...
    def generate_patch(self, type):
        ...
        """
        img2=Image.open('gen_adv_voc3/999_3.png').convert('RGB')
