...
#训练新的类别对抗块，注意这里针对的是VOC数据集的（只需更改类别数20/80及cfg/下网络结构即可），
...
class PatchTrainer(object):
    def __init__(self, mode):
        ...
        self.prob_extractor = MaxProbExtractor(0, 20, self.config).cuda()   #voc
        ...
        for epoch in range(n_epochs):
            ...
            for i_batch, (img_batch, lab_batch) in tqdm(enumerate(train_loader), desc=f'Running epoch {epoch}',
                ...
                with autograd.detect_anomaly():  #检测梯度异常值：无穷大/NAN(梯度爆炸/消失)
                    ...
                    ##    file_name='gen_adv_voc0/'+str(epoch)+'_'+'0.png'
                    #    img.save(file_name)
#                     #print('=====2======:',img.size())
#                     plt.imshow(img)
#                     plt.show()
...
#                     plt.show()
            ...
            #im_w=np.random.randint(20,60)
            #im_h=np.random.randint(20,60)
            ...
            if True:
                ...
                finame='gen_adv_voc0/'+str(epoch)+'_0.png'
                im.save(finame)
                del adv_batch_t, output, max_prob, det_loss, p_img_batch, nps_loss, tv_loss, loss
                torch.cuda.empty_cache()
            et0 = time.time()
