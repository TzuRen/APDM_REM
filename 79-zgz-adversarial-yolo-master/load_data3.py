#encoding=utf-8
#相比于load_data，我想改变其类别loss，反着来，训练一个专门代表人的图像块  77行改为取反
import fnmatch
import math
import os
import sys
...
class MaxProbExtractor(nn.Module):
    ...
    def __init__(self, cls_id, num_cls, config):# 0，80，Iria总共80类，人是第0类？？
    ...
    def forward(self, YOLOoutput):
        ...
        #做出更改：由max----》min
        max_conf_k, max_conf_idx = confs_if_object.topk(4, dim=1)
        max_conf = torch.sum(max_conf_k, dim=1)  #返回相应行最大值，及其对应索引
        
        #max_conf, max_conf_idx = torch.min(confs_if_object, dim=1) 
