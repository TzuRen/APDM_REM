...
class PatchTrainer(object):
    def __init__(self, mode):
        ...
        self.darknet_model = Darknet(self.config.cfgfile)  #cfg/yolo.cfg  80类
        self.darknet_model.load_weights(self.config.weightfile)
        self.darknet_model = self.darknet_model.eval().cuda() # TODO: Why eval?
        self.patch_applier = PatchApplier().cuda()
        self.patch_transformer = PatchTransformer().cuda()
        #第一个参数0，指定类别为人，可人为更改，如1
        #self.prob_extractor = MaxProbExtractor(1, 80, self.config).cuda()
        self.prob_extractor = MaxProbExtractor(14, 20, self.config).cuda()
...
#                    img = p_img_batch[1, :, :,]
 ...
 #                   img = transforms.ToPILImage()(img.detach().cpu())
           #         print('=====2======:',img.size())
            ...
            finame='gen_adv_inria_tiny_14/'+str(14)+'_'+str(epoch)+'.png'
            im.save(finame)
            #plt.imshow(im)
            ...
            if True:
                ...
                #plt.show()
                #print('img save -----------------------------------------')
                #im.save("class1_patches/{time_str}_{self.config.patch_name}_{epoch}.png")
                #finame='class1_patches/'+str(time_str)+'_'+str(epoch)+'.png'
                #im.save(finame)
                del adv_batch_t, output, max_prob, det_loss, p_img_batch, nps_loss, tv_loss, loss
                torch.cuda.empty_cache()
            et0 = time.time()
