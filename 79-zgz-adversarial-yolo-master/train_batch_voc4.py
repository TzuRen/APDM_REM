...
class PatchTrainer(object):
    def __init__(self, mode):
        ...
        self.prob_extractor = MaxProbExtractor2(2, 20, self.config).cuda()   #coco
        #self.prob_extractor = MaxProbExtractor2(13, 20, self.config).cuda()   #voc
    ...
    def train(self):
        ...
        img_path='/home/zgz/pytorch-yolo2-master/my_VOC/2.txt'
        
        train_loader = torch.utils.data.DataLoader(InriaDataset2(img_path, max_lab, img_size,shuffle=True),
        ...
        for epoch in range(n_epochs):
            ...
            #for i_batch, (img_batch, lab_batch) in enumerate(train_loader):
                ...
                #提取人的最大类别概率
                max_conf,max_cls = self.prob_extractor(output,epoch)
                #max_conf = self.prob_extractor(output,epoch)
                ...
                loss = det_loss+ nps_loss + cls_loss+torch.max(tv_loss, torch.tensor(0.1).cuda())
                if kk%30==0:
                ...
                if kk%100==0:
                    finame='gen_adv_tiny_2/'+str(2)+'_'+str(epoch)+'_'+str(kk)+'.png'
                    im.save(finame)
                ...
                if kk + 1 >= len(train_loader):
                    ...
                    del adv_batch_t, output, max_conf,det_loss, max_cls,cls_loss,p_img_batch, nps_loss, tv_loss, loss
            ...
            #    finame='gen_adv_all/'+str(epoch//100)+'_'+str(epoch)+'.png'
             #   im.save(finame)
                ...
                if type(adv_batch_t) != int:
                    del adv_batch_t, output,det_loss,max_conf,p_img_batch, nps_loss, tv_loss, loss
                torch.cuda.empty_cache()
    ...
    def generate_patch(self, type):
        ...
        '''
        img2=Image.open('gen_adv_all/1_188_700.png').convert('RGB')
