...
class MaxProbExtractor(nn.Module):
    ...
    def forward(self, YOLOoutput):
        ...
        #print('-----confs_for_class-----',confs_for_class)
...
class PatchApplier(nn.Module):
    ...
    def forward(self, img_batch, adv_batch):
        ...
        for adv in advs:
            ...
            #应位置的值，然后返回替换后的整体值
            print('---------------adv size----- : ',img_batch.shape,adv.shape)
...
class InriaDataset2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print('img_path',img_path)
        #lab_path = os.path.join(self.lab_dir, self.img_names[idx]).replace('.jpg', '.txt').replace('.png', '.txt')
        lab_path = img_path.replace('images', 'labels').replace('JPEGImages', 'labels').replace('.jpg', '.txt').replace('.png','.txt')
...
class MyInriaDataset(Dataset):
    ...
    def __init__(self, root, max_lab, imgsize, shuffle=True):
        '''
        n_png_images = len(fnmatch.filter(os.listdir(img_dir), '*.png'))
        n_jpg_images = len(fnmatch.filter(os.listdir(img_dir), '*.jpg'))
        n_images = n_png_images + n_jpg_images
    ...
    def __getitem__(self, idx):
        ...
        img_path = self.lines[idx].rstrip()
        image = Image.open(img_path).convert('RGB')
        #lab_path = img_path.replace('images', 'labels').replace('JPEGImages',
        #        'labels').replace('.jpg', '.txt').replace('.png','.txt')
        lab_path=img_path.replace('/img4/','/img4_pred_label/').replace('.jpg','.txt')
        if os.path.getsize(lab_path):       #check to see if label file contains data. 
            label = np.loadtxt(lab_path)
        else:
            label = np.ones([5])
