...
datacfg='cfg/my_voc_repair.data'  #相应做了一系列修改，
...
def train(epoch):
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.listDataset3(testlist, shape=(init_width,init_height),
                       shuffle=True,
                       transform=transforms.Compose([
                           transforms.ToTensor()
                       ]), 
    ...
    for epoch in range(0,1):
        ...
        loss=0
        fw=codecs.open('/home/zgz/pytorch-yolo2-master/video_dispaly/img4_adv.txt','w')
        for batch_idx, (img_name,data, target) in enumerate(train_loader):
            
            #print('--------------------data---------------',type(data),data.shape) #<class 'torch.Tensor'> (64, 3, :, :)
            
            for kk in range(batch_size):
            
                im = transforms.ToPILImage('RGB')(data[kk,:,:,:])
                #print(img_name)
                im.save('/home/zgz/pytorch-yolo2-master/video_dispaly/img4_adv/'+img_name[kk])
                fw.write('/home/zgz/pytorch-yolo2-master/video_dispaly/img4_adv/'+img_name[kk]+'\n')
