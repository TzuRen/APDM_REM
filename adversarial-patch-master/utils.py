...
#_, term_width = os.popen('stty size', 'r').read().split()  #获取控制台大小
...
def circle_transform(patch, data_shape, patch_shape, image_size):
    ...
    #print('---------------',random_x,'============',random_y,'+++++++++',patch_shape[-1])  #patch_shape[-1]=74, adv patch size=74*74  
    mask = np.copy(x)
    mask[mask != 0] = 1.0
    
    return x, mask, patch.shape,random_y+patch_shape[-1]/2,random_x+shape[-1]/2
