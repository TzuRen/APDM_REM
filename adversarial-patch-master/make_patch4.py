...
parser.add_argument('--outf', default='./resnet18_adv', help='folder to output images and model checkpoints')
...
img_dir='/home/zgz/adversarial_samples/adversarial-patch-master2/adversarial-patch-master/imagenet/val1'

train_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(img_dir, transforms.Compose([
...
def train(epoch, patch, patch_shape):
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        if patch_type == 'circle':
            patch, mask, patch_shape,center_x,center_y = circle_transform(patch, data_shape, patch_shape, image_size)
        ...
        if adv_label == target2:
            ...
            if plot_all == 1: 
                ...
                #vutils.save_image(mask.data, "./%s/%d_%d_original.png" %(opt.outf, batch_idx, ori_label), normalize=True)
                # plot adversarial image
                vutils.save_image(adv_x.data, "./%s/%d_%d_%d.png" %(opt.outf, batch_idx, ori_label,adv_label), normalize=True)
        ...
        # log to file 可前台打印进度条，不可使用nohup  
...
def attack(x, patch, mask):
    ...
    while conf_target > target_prob:
        ...
        target_prob = out.data[0][target2]
    ...
    return adv_x, mask, patch,target2 
